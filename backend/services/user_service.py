...
class UserService:
    def __init__(self, config, repository=None):
        ...
        self.jwt_access_expires = int(config.JWT_ACCESS_TOKEN_EXPIRES.total_seconds())
        self.jwt_refresh_expires = int(config.JWT_REFRESH_TOKEN_EXPIRES.total_seconds())
    ...
    def register_user(self, username, email, password, role="regular"):
        if self.repo.find_by_email(email):
            logger.warning(
                "Attempted to register user with existing email", extra={"email": email}
            )
        ...
        logger.info(
            "User registered successfully", extra={"email": email, "user_id": user_id}
        )
    ...
    def login_user(self, email, password):
        ...
        if not user:
            logger.warning(
                "Login attempt for non-existent user", extra={"email": email}
            )
        ...
        access_token = jwt.encode(
            access_payload, self.jwt_secret_key, algorithm=self.jwt_algorithm
        )
        refresh_payload = {
            "sub": user["username"],
            "email": user["email"],
        ...
        refresh_token = jwt.encode(
            refresh_payload, self.jwt_secret_key, algorithm=self.jwt_algorithm
        )
    ...
    def refresh_access_token(self, refresh_token):
        try:
            payload = jwt.decode(
                refresh_token, self.jwt_secret_key, algorithms=[self.jwt_algorithm]
            )
        ...
        stored_hash_bytes = (
            ...
            else (stored_hash.encode("utf-8") if stored_hash else None)
        )
        if not stored_hash_bytes or not bcrypt.checkpw(
            refresh_token.encode("utf-8"), stored_hash_bytes
        ):
        ...
        if not user:
            logger.error(
                "User not found during token refresh", extra={"username": username}
            )
            return {"error": "User not found"}
        now = datetime.datetime.now(timezone.utc)
        new_access_payload = {
        ...
        new_access_token = jwt.encode(
            new_access_payload, self.jwt_secret_key, algorithm=self.jwt_algorithm
        )
        new_refresh_payload = {
            "sub": user["username"],
            "email": user["email"],
        ...
        new_refresh_token = jwt.encode(
            new_refresh_payload, self.jwt_secret_key, algorithm=self.jwt_algorithm
        )
        new_hashed_refresh = bcrypt.hashpw(
