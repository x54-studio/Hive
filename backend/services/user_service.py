...
class UserService:
    def __init__(self, config, repository=None):
        ...
        self.jwt_access_expires = int(
            config.JWT_ACCESS_TOKEN_EXPIRES.total_seconds()
        )
        self.jwt_refresh_expires = int(
    ...
    def register_user(self, username, email, password, role="regular"):
        if self.repo.find_by_email(email):
            logger.warning(
                "Attempted to register user with existing email",
                extra={"email": email}
            )
    ...
    def login_user(self, email, password):
        ...
        if not bcrypt.checkpw(password.encode("utf-8"), user["password"]):
            logger.warning(
                "Invalid credentials for login", extra={"email": email}
            )
        ...
        logger.info(
            "User logged in successfully", extra={"email": email}
        )
        return {
    ...
    def refresh_access_token(self, refresh_token):
        try:
            payload = jwt.decode(
                refresh_token, self.jwt_secret_key,
        ...
        stored_hash_bytes = (
            stored_hash if isinstance(stored_hash, bytes)
        ...
        if (not stored_hash_bytes or not bcrypt.checkpw(
                ...
                stored_hash_bytes)):
            logger.warning(
                "Refresh token mismatch", extra={"username": username}
            )
            return {"error": "Invalid refresh token"}
        ...
        logger.info(
            "Refreshed tokens for user", extra={"username": username}
        )
        return {
